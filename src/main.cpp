...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    if (HASH != __hyprland_api_get_client_hash()) {
        std::string mismatchMsg = "[hyprscroller] Version mismatch: headers ver [" + std::string(__hyprland_api_get_client_hash()) + "] vs running hyprland ver [" + HASH + "]";
        HyprlandAPI::addNotification(PHANDLE, mismatchMsg, CHyprColor{1.0, 0.2, 0.2, 1.0}, 15000);
        throw std::runtime_error(mismatchMsg);
    }
    ...
    // one value out of: { oneeighth, onesixth, onefourth, onethird, threeeighths, onehalf (default), fiveeighths, twothirds, threequarters, fivesixths, seveneighths, one }
    ...
    // one value used when a column has a single window; default full height
    // values: oneeighth..one, default: one
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:scroller:single_row_height", Hyprlang::STRING{"one"});
    ...
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:scroller:avoid_focus_on_float_close", Hyprlang::INT{0});// 0, 1
